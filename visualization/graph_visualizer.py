...
def color_unused_nodes_grey(G, pos, values_dict, node_size=2000):
    # Find the nodes that are not in the dictionary
    unused_nodes = [node for node in G.nodes if node not in values_dict.keys()]
    ...
    unused_edges = [(u, v) for u, v in G.edges if (u not in values_dict.keys()) or (v not in values_dict.keys())]
